...
export const newUser = TryCatch(
  async (
    ...
    try {
      ...
      if (!name || !email || !photo || !gender || !dob) return next(new ErrorHanlder("Please Add all fileds",400));
...
export const singleUser = TryCatch(async(req,res,next)=>{
  ...
  if(!singleData) return res.json({success:false,message:"No Data found",code:400})
...
export const deleteUser = TryCatch(async (req,res,next)=>{
  ...
  if(!deleteData) return res.json({success:false,message:"No Data found",code:400})
